...
int update_SDUT(UPDATE_FUNC_ARGS) {
			...
			for (ry=-1; ry<2; ry++)
				if (x+rx>=0 && y+ry>0 && x+rx<XRES && y+ry<YRES && (rx || ry))
				{
					...
					if ((r>>8)>=NPART || !r || (parts[i].life >=maxlife))
						continue;
					if ((r&0xFF)==PT_SDUT&&33>=rand()/(RAND_MAX/100)+1)
					{
			...
			for (ry=-1; ry<2; ry++)
				if (x+rx>=0 && y+ry>0 && x+rx<XRES && y+ry<YRES && (rx || ry))
				{
					...
					if ((bmap[(y+ry)/CELL][(x+rx)/CELL]==WL_WALLELEC||bmap[(y+ry)/CELL][(x+rx)/CELL]==WL_EWALL||bmap[(y+ry)/CELL][(x+rx)/CELL]==WL_DESTROYALL||bmap[(y+ry)/CELL][(x+rx)/CELL]==WL_WALL||
						...
						continue;
					if ((!r)&&parts[i].life>=1)//if nothing then create SDUT
					{
						np = create_part(-1,x+rx,y+ry,PT_SDUT);
						if (np<0) continue;
						parts[i].life--;
	...
	for ( trade = 0; trade<4; trade ++)
	{
		...
		if (x+rx>=0 && y+ry>0 && x+rx<XRES && y+ry<YRES && (rx || ry))
		{
			...
			if ((r>>8)>=NPART || !r)
				continue;
			if ((r&0xFF)==PT_SDUT&&(parts[i].life>parts[r>>8].life)&&parts[i].life>0)//diffusion
			{
