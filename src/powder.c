...
static int eval_move(int pt, int nx, int ny, unsigned *rr)
{
    ...
    if((pt==PT_PHOT||pt==PT_MUPT)&&(	                    // < AntB
                (r&0xFF)==PT_MUPT || (r&0xFF)==PT_MUPT ||	// < Edit
...
void update_particles_i(pixel *vid, int start, int inc)
{
    ...
    for(i=start; i<(NPART-starti); i+=inc)
        if(parts[i].type)
        {
            ...
            if(parts[i].life && t!=PT_ACID  && t!=PT_COAL && t!=PT_WOOD && t!=PT_NBLE && t!=PT_SWCH && t!=PT_STKM && t!=PT_STKM2 && t!=PT_FUSE && t!=PT_FSEP && t!=PT_BCOL && t!=PT_GOL && t!=PT_CRAC && t!=PT_DEUT && t!=PT_RADI && t!=PT_DBTY && t!=PT_NP2) //AntB Edit - Ignore life value
            {
                if(!(parts[i].life==10&&(parts[i].type==PT_LCRY||parts[i].type==PT_PCLN||parts[i].type==PT_HSWC||parts[i].type==PT_PUMP||parts[i].type==PT_RNEO||parts[i].type==PT_GNEO||parts[i].type==PT_BNEO||parts[i].type==PT_CNEO||parts[i].type==PT_MNEO||parts[i].type==PT_YNEO||parts[i].type==PT_SBTY)) && !(parts[i].life%4==0 && parts[i].type==PT_INST))
                    parts[i].life--;
                if(parts[i].life<=0 && t!=PT_METL && t!=PT_ARAY && t!=PT_IRON && t!=PT_FIRW && t!=PT_PCLN && t!=PT_HSWC && t!=PT_PUMP && t!=PT_WATR && t!=PT_RBDM && t!=PT_LRBD && t!=PT_SLTW && t!=PT_BRMT && t!=PT_PSCN && t!=PT_NSCN && t!=PT_NTCT && t!=PT_PTCT && t!=PT_BMTL && t!=PT_SPRK && t!=PT_LAVA && t!=PT_ETRD&&t!=PT_LCRY && t!=PT_INWR && t!=PT_QRTZ && t!=PT_GLOW && t!= PT_FOG && t!=PT_PIPE && t!=PT_FRZW &&(t!=PT_ICEI&&parts[i].ctype!=PT_FRZW)&&t!=PT_INST && t!=PT_SHLD1&& t!=PT_SHLD2&& t!=PT_SHLD3&& t!=PT_SHLD4 && //AntB Edit
                ...
                {
                    t = parts[i].ctype;
                    if(t!=PT_METL&&t!=PT_IRON&&t!=PT_BMTL&&t!=PT_BRMT&&t!=PT_LRBD&&t!=PT_RBDM&&t!=PT_BTRY&&t!=PT_NBLE&&t!=PT_QRTZ&&t!=PT_DBTY&&t!=PT_SBTY)  //AntB Edit - Maintain temps
		...
		if(parts[i].temp>747.0f)
            ...
            if(t==PT_SPRK&&parts[i].ctype==PT_ETRD&&parts[i].life==1)
            {
                ...
                if(nearp!=-1&&(parts_avg(i, nearp, PT_INSL)!=PT_INSL||parts_avg(i, nearp, PT_RBRI)!=PT_RBRI)) //AntB Edit
                    ...
                    for(ny=-2; ny<3; ny++)
                                ...
                                x+nx<XRES && y+ny<YRES && (nx || ny))
                        {
                            ...
                            if((r>>8)>=NPART || !r)
                                continue;
                            rt = parts[r>>8].type;
                            if(parts_avg(i,r>>8,PT_INSL) != PT_INSL && parts_avg(i,r>>8,PT_RBRI) != PT_RBRI) // AntB edit

                            {
                                if((rt==PT_METL||rt==PT_IRON||rt==PT_ETRD||rt==PT_BMTL||rt==PT_BRMT||rt==PT_LRBD||rt==PT_RBDM||rt==PT_PSCN||rt==PT_NSCN||rt==PT_NBLE||rt==PT_SUWR||rt==PT_LEAD||rt==PT_LLED||rt==PT_ZINC||rt==PT_GSTL)&&parts[r>>8].life==0 && abs(nx)+abs(ny) < 4) //AntB Edit - BTRY Conductivity
                                {
                                    parts[r>>8].life = 4;
                                    parts[r>>8].ctype = rt;
                                    parts[r>>8].type = PT_SPRK;
...
#include "mod/ab-reactions.inc"
                    ...
                    for(ny=-2; ny<3; ny++)
                                ...
                                x+nx<XRES && y+ny<YRES && (nx || ny))
                        {
                            ...
                            if(rt==PT_SPRK && parts[r>>8].ctype == PT_SWCH && t==PT_SPRK)
                            {
                                ...
                                if(parts[i].ctype == PT_NSCN&&(parts_avg(r>>8, i,PT_INSL) != PT_INSL || parts_avg(r>>8, i,PT_RBRI) != PT_RBRI))
                                {
                            ...
                            if(rt==PT_SWCH && t==PT_SPRK)
                            {
                                ...
                                if(parts[i].ctype == PT_PSCN&&(parts_avg(r>>8, i,PT_INSL) != PT_INSL || parts_avg(r>>8, i,PT_RBRI) != PT_RBRI) && parts[r>>8].life<10) //AntB Edit
                                    parts[r>>8].life = 10;
                                if(parts[i].ctype == PT_NSCN&&(parts_avg(r>>8, i,PT_INSL) != PT_INSL || parts_avg(r>>8, i,PT_RBRI) != PT_RBRI)) //AntB Edit
                                    parts[r>>8].life = 9;
                                if(!(parts[i].ctype == PT_PSCN||parts[i].ctype == PT_NSCN)&&parts[r>>8].life >= 10&&(parts_avg(r>>8, i,PT_INSL) != PT_INSL || parts_avg(r>>8, i,PT_RBRI) != PT_RBRI)) //AntB Edit
                                {
                                    parts[r>>8].type = PT_SPRK;
                            ...
                            if(parts_avg(r>>8, i,PT_INSL) != PT_INSL && parts_avg(r>>8, i,PT_RBRI) != PT_RBRI) //AntB Edit
                            {
                                ...
                                if(t==PT_SPRK && rt==PT_NTCT && parts[r>>8].life==0 &&
                                        (parts[i].life<3 || ((r>>8)<i && parts[i].life<4)) && abs(nx)+abs(ny)<4)
                                {
                                    if((parts[i].ctype==PT_NSCN||parts[i].ctype==PT_NTCT||(parts[i].ctype==PT_PSCN&&parts[r>>8].temp>373.0f))&&(pavg != PT_INSL || pavg != PT_RBRI)) //AntB Edit
                                    {
                                        parts[r>>8].type = PT_SPRK;
                                        parts[r>>8].life = 4;
                                        parts[r>>8].ctype = rt;
                                ...
                                if(t==PT_SPRK && rt==PT_PTCT && parts[r>>8].life==0 &&
                                        (parts[i].life<3 || ((r>>8)<i && parts[i].life<4)) && abs(nx)+abs(ny)<4)
                                {
                                    if((parts[i].ctype==PT_NSCN||parts[i].ctype==PT_PTCT||(parts[i].ctype==PT_PSCN&&parts[r>>8].temp<373.0f))&&(pavg != PT_INSL || pavg != PT_RBRI)) //AntB Edit
                                    {
                                        parts[r>>8].type = PT_SPRK;
                                        parts[r>>8].life = 4;
                                        parts[r>>8].ctype = rt;
                                ...
                                if(t==PT_SPRK && rt==PT_INWR && parts[r>>8].life==0 &&
                                        (parts[i].life<3 || ((r>>8)<i && parts[i].life<4)) && abs(nx)+abs(ny)<4)
                                {
                                    if((parts[i].ctype==PT_NSCN||parts[i].ctype==PT_INWR||parts[i].ctype==PT_PSCN)&&(pavg != PT_INSL || pavg != PT_RBRI)) //AntB Edit
                                    {
                                        parts[r>>8].type = PT_SPRK;
                                        parts[r>>8].life = 4;
                                        parts[r>>8].ctype = rt;
	...
	/*     |=>	 */			if(t==PT_SPRK && (rt==PT_LEAD || rt==PT_LLED || rt==PT_GSTL) && parts[r>>8].life==0 &&
	/*     |	 */				(parts[i].life<2 || ((r>>8)<i && parts[i].life<3)) && abs(nx)+abs(ny)<4)
	/*	   |	 */			{
	/*	   |	 */				if(parts_avg(r>>8, i,PT_INSL) != PT_INSL || parts_avg(r>>8, i,PT_RBRI) != PT_RBRI)
	/* AntB Edit */				{
	/*	   |	 */						parts[r>>8].type = PT_SPRK;
	/*	   |	 */						parts[r>>8].life = 6;
	/*	   |	 */						parts[r>>8].ctype = rt;
