...
#include "mod/ab-life.inc"
    ...
    if(t!=PT_STKM)// && t!=PT_PHOT && t!=PT_NEUT && t!=PT_MUPT && t!= PT_MUNE) -- Breaks floodfill // AntB Edit
        pmap[y][x] = t|(i<<8);
    else if(t==PT_STKM)
    {
        if(isplayer==0)
...
void update_particles_i(pixel *vid, int start, int inc)
{
    ...
    for(i=start; i<(NPART-starti); i+=inc)
        if(parts[i].type)
        {
            ...
            if(parts[i].life && t!=PT_ACID  && t!=PT_COAL && t!=PT_WOOD && t!=PT_NBLE && t!=PT_SWCH && t!=PT_STKM && t!=PT_FUSE && t!=PT_FSEP && t!=PT_BCOL && t!=PT_GOL && t!=PT_CRAC && t!=PT_DEUT && t!=PT_RADI && t!=PT_DBTY && t!=PT_NP2) //AntB Edit - Ignore life value
            {
                ...
                {
                    t = parts[i].ctype;
                    if(t!=PT_METL&&t!=PT_IRON&&t!=PT_BMTL&&t!=PT_BRMT&&t!=PT_LRBD&&t!=PT_RBDM&&t!=PT_BTRY&&t!=PT_NBLE&&t!=PT_DBTY&&t!=PT_SBTY)  //AntB Edit - Maintain temps
