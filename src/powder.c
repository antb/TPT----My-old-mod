    ...
    for(i=start; i<(NPART-starti); i+=inc)
        if(parts[i].type)
        {
            ...
            if(parts[i].life && t!=PT_ACID  && t!=PT_COAL && t!=PT_WOOD && t!=PT_NBLE && t!=PT_SWCH && t!=PT_STKM && t!=PT_FUSE && t!=PT_FSEP && t!=PT_BCOL && t!=PT_RADI) //AntB Edit
            {
                ...
                {
                    t = parts[i].ctype;
                    if(t!=PT_METL&&t!=PT_IRON&&t!=PT_BMTL&&t!=PT_BRMT&&t!=PT_LRBD&&t!=PT_RBDM&&t!=PT_BTRY&&t!=PT_NBLE&&t!=PT_SUWR&&t!=PT_LEAD&&t!=PT_LLED) //AntB Edit (elec)
		...
		t = parts[i].type = PT_BRMT;
            ...
            if(t==PT_SPRK&&parts[i].ctype==PT_ETRD&&parts[i].life==1)
            {
                ...
                if(nearp!=-1&&(parts_avg(i, nearp)!=PT_INSL || parts_avg(i, nearp)!=PT_RBRI))  //AntB Edit
                {
            ...
            if(t==PT_WATR || t==PT_ETRD || t==PT_SLTW || t==PT_METL || t==PT_IRON || t==PT_RBDM || t==PT_LRBD || t==PT_BRMT || t==PT_PSCN || t==PT_NSCN || t==PT_NTCT || t==PT_PTCT || t==PT_BMTL || t==PT_SPRK|| t == PT_NBLE || t==PT_INWR || t==PT_SUWR || t==PT_LEAD || t==PT_LLED) //AntB Edit (elec)
            {
                nx = x % CELL;
                if(nx == 0)
                    nx = x/CELL - 1;
                ...
                {
                    if(t==PT_WATR || t==PT_ETRD || t==PT_SLTW || t==PT_METL || t==PT_IRON || t==PT_RBDM || t==PT_LRBD || t==PT_NSCN || t==PT_NTCT || t==PT_PTCT || t==PT_PSCN || t==PT_BRMT || t==PT_BMTL||t==PT_NBLE || t==PT_INWR || t==PT_SUWR || t==PT_LEAD || t==PT_LLED) //AntB Edit (elec)
                    {
            ...
            else if(t==PT_PHOT || t==PT_MUPT) //AntB Edit
            {
                rt = 3 + (int)pv[y/CELL][x/CELL];
                for(nx=0; nx<1; nx++)
                    for(ny=0; ny<1; ny++)
                    ...
                    for(ny=-2; ny<3; ny++)
                                ...
                                x+nx<XRES && y+ny<YRES && (nx || ny))
                        {
                            ...
                            if((r>>8)>=NPART || !r)
                                continue;
                            rt = parts[r>>8].type;
                            if(parts_avg(i,r>>8) != PT_INSL && parts_avg(i, r>>8)!=PT_RBRI) // AntB edit
                            {
...
#include "ab-reactions.inc"
                    ...
                    for(ny=-2; ny<3; ny++)
                                ...
                                x+nx<XRES && y+ny<YRES && (nx || ny))
                        {
                            ...
                            if(bmap[(y+ny)/CELL][(x+nx)/CELL] && bmap[(y+ny)/CELL][(x+nx)/CELL]!=5)
                                continue;
                            rt = parts[r>>8].type;
                            if((a || ptypes[rt].explosive) && ((rt!=PT_RBDM && rt!=PT_LRBD && rt!=PT_INSL && rt!=PT_RBRI && rt!=PT_SWCH) || t!=PT_SPRK) &&  //AntB Edit
                            ...
                            {
                                if(t==PT_SPRK && (rt==PT_METL||rt==PT_IRON||rt==PT_ETRD||rt==PT_BMTL||rt==PT_BRMT||rt==PT_LRBD||rt==PT_RBDM||rt==PT_PSCN||rt==PT_NSCN||rt==PT_NBLE||rt==PT_SUWR) && parts[r>>8].life==0 && //AntB Edit (elec)
	...
	/*     |=>	 */			if(t==PT_SPRK && (rt==PT_LEAD || rt==PT_LLED) && parts[r>>8].life==0 &&
	...
	/*	   |=>	 */			}
                                ...
                                {
                                    if(rt==PT_METL||rt==PT_ETRD||rt==PT_BMTL||rt==PT_BRMT||rt==PT_LRBD||rt==PT_RBDM||rt==PT_PSCN||rt==PT_NSCN||rt==PT_SUWR||rt==PT_LEAD||rt==PT_LLED) //AntB Edit (elec)
                                    {
                                        t = parts[i].type = PT_ETRD;
                                        parts[i].ctype = PT_NONE;
                                        parts[i].life = 20;
...
killed:
            ...
            if(parts[i].type==PT_PCLN)
            {
                ...
                if(parts[i].ctype && parts[i].life==10) {
                    if(parts[i].ctype==PT_PHOT || parts[i].ctype==PT_MUPT) { //AntB Edit
                        for(nx=-1; nx<2; nx++) {
                            for(ny=-1; ny<2; ny++) {
